...
%this.
%\begin{figure}[!tbp]
%  \includegraphics[width=\textwidth/2]{figures/paperOrganization}
...
%amortized~\cite{summa}. The bandwidth cost is $Q_{comm} = (a+b)K$ (or $Q_{comm}
%= (M+N)K/\sqrt{p}$) and the latency cost $L_{comm} = 2t$ (or $2\sqrt{p}$). The
%memory requirement per process is $S \ge (MN + MK + NK)/p$. 
%
...
%
% The rules are as follows. First,
...
%\macb{Intuition}
%%
%Intuitively, the $S$-partition technique may be seen as a generalization of the
%Loomis-Whitney inequality~\cite{loomisWhitney}, which is used in linear algebra
%for bounding the amount of I/O~\cite{loomisApplied}: both techniques aim at
%  finding the optimal surface (communication) to volume (computation) ratio in
%  a given setting. 
%%
...
%\mac{This subsection seems very detached from the rest. I strongly 
%suggest
%a clear motivation in the first 1-2 sentences on WHY we need this 
%part
...
projection onto plane \text{ij}: $\phi_{ij}(V_i)$. Denote those projections 
...
either in \text{i}, \text{j} or \text{k} direction (or a direction not 
orthogonal to any of those vectors). We then reuse only the data that is "on 
our way" - if we move in \text{i} we reuse inputs from B, moving in \text{j} 
reuses inputs from A, moving in \text{k} reuses partial results of C. Moving in 
...
Note that $\alpha_i$ and 
 ...
 the 
 dependency direction \text{k}, immediately "consuming" (reusing) the outputs 
...
%  decomposition~\cite{25d}.
%  \item CARMA~\cite{CARMA} asymptotically reaches \textbf{cubic} 
%  scheme, 
%  guaranteeing that 
%  the longest dimension of a local cuboidal domain is at most two times 
%  larger than the 
...
%
All implementations were compiled using XX compiler. We use YY implementation 
of MPI. The intra-node parallelism is handled internally by the BLAS routines 
(no OpenMP calls). 
...
%
%\begin{algorithm}
%\caption{$COMM^{2}$ - Matrix Multiplication Algorithm} \label{algorithm}
%\begin{algorithmic}[1]
%\Require $\text{input matrices: } A \in \mathbb{R}^{M \times K}, B \in 
%\mathbb{R}^{K \times N}, \text{num. of proc.: } p, \text{mem. size: } S, 
%\text{comm. granularity: } t $
%\Ensure $\text{output matrix: } C = AB \in \mathbb{R}^{M \times N}$
%\State $a_{opt} \gets \left \lfloor \min\Big\{\sqrt{S}, 
%\Big(\frac{MNK}{p}\Big)^{1/3} \Big\} \right \rfloor$ 
%\Comment{get optimal local domain shape (Equation \ref{eq:optTileShape})}
%\State $b_{opt} \gets \left \lfloor \max\Big\{\frac{MNK}{pS}, 
%\Big(\frac{MNK}{p}\Big)^{1/3} \Big\} \right \rfloor$
%\State $(g_M, g_N, g_K, a, b) \gets FitRanks(M,N,K,a,b,p)$ \Comment{get the 
%global 3D domain decomposition}
%
%\State $(map_A, map_B, map_C) \gets GetDataDecomp(A,B, g_M, g_N, g_K)$ 
%\Comment{get the data decomposition}
%
%\For{$p_i \in \left\{1 \dots p \right\}$} %\textbf{in parallel}
%%
%\State $(r_m,r_n, r_k) \gets GetProcRanks(g_M, g_N,g_K,p)$ \Comment{translate 
%proc. rank to the 3D grid}
%\State $(m_s,m_e) \gets (r_m a, \min\{(r_m+1) a, M\})$ \Comment{get local 
%domain ranges}
%\State $(n_s,n_e) \gets (r_n a, \min\{(r_n+1) a, N\})$
%\State $(k_s,k_e) \gets (r_k a, \min\{(r_k+1) b, K\})$
%%\If {comm = two\_sided}
%%\State $P \gets P.split\_cyclicaly\_by(d_i)$ \Comment {Split processors 
%%cyclically into groups}
%%\If {$S.m\_split(i)$} \Comment{if $m$ was split}
%%\State $A.split\_by(d_i)$
%%\State $B.all\_gather(P)$ \Comment{Perform all-gather copy of $B$ within each 
%%processor group} 
%%\ElsIf {$S.n\_split(i)$} \Comment{if $n$ was split}
%%\State $B.split\_by(d_i)$
%%\State $A.all\_gather(P)$ \Comment{Perform all-gather copy of $A$ within each 
%%processor group} 
%%\EndIf
%%\Else
%%\State Assign all processors to each subproblem sequentially.
%%\EndIf 
%\State $C_l \gets ComputeSequential([m_s,m_e], [n_s,n_e], [k_s,k_e], map_A, 
%map_B)$ \Comment{Evaluate local domain}
%\State $C_l \gets Reduce(C_l, map_C)$ \Comment{Reduce the partial results}
%\EndFor
%\end{algorithmic}
%\end{algorithm}
%
%\begin{algorithm}
%\caption{$GetDataDecomp$} \label{algorithm}
%\begin{algorithmic}[1]
%\Require $\text{input matrices: } A \in \mathbb{R}^{M \times K}, B \in 
%\mathbb{R}^{K \times N}, \text{3D grid process decomposition: } g_M, g_N, g_K$
%\Ensure Mapping functions from global index of input to the owner's rank and 
%its local index: 
%\Statex $map_A,~map_B:~[g_i, g_j]~\rightarrow~[r, l_i, l_j]$
%\State $DoSomething$
%\end{algorithmic}
%\end{algorithm}
%
%\begin{figure}
%\begin{algorithm}
%\caption{ComputeSequential} \label{algorithm}
%\begin{algorithmic}[1]
%\Require $\text{index ranges: } (m_s,m_e), (n_s,n_e), (k_s,k_e), \text{mapping 
%functions: } map_A, map_B, \text{ chunk size: } t$
%\Ensure $\text{partial results of C: } C_l = A_l B_l \in \mathbb{R}^{(m_e - 
%m_s) \times (n_e - n_s)}$
%\State $(t_s, t_e) \gets (k_s, k_s + t)$ \Comment{Initialize first chunk's 
%indices}
%\State $C \gets \mathbf{0^{(m_e - m_s) \times (n_e - n_s)}}, ChunkA \gets 
%\mathbf{0^{(m_e - m_s) \times t}}, ChunkB \gets \mathbf{0^{t \times (n_e - 
%n_s)}}$ \Comment{Initialize receive buffers}
%\While{$t_e \le k_e$}
%\State $ChunkA_{next} \gets AsyncReceiveA([m_s, m_e], [t_s, t_e])$ 
%\Comment{Start receiving  the next chunk}
%\State $ChunkB_{next} \gets AsyncReceiveB([t_s, t_e], [n_s, n_e])$ 
%\State $C \gets C + ChunkA \cdot ChunkB$
%\State $(t_s, t_e) \gets (t_e, \min\{t_e + t, k_e\})$ 
%\State $WaitAll(ChunkA_{next}, ChunkB_{next})$ \Comment{When the computation 
%of 
%previous chunk is done, wait for the next one}
%\State $ChunkA \gets ChunkA_{next}$ \Comment{Swap the buffers}
%\State $ChunkB \gets ChunkB_{next}$ \Comment{Swap the buffers}
%\EndWhile
%\end{algorithmic}
%\end{algorithm}
%\caption{$\mathbf{0^{a \times b}}$ is an $a \times b$ zero matrix.}
%\end{figure}
%
%
%\begin{figure}
%\begin{algorithm}
%\caption{FitRanks} \label{algorithm}
%\begin{algorithmic}[1]
%\Require $\text{matix sizes: } M,N,K, \text{ optimal domain size: } 
%a_{opt},b_{opt}, \text{ num procs: } p,  \text{ computation to communication 
%cost ratio: } \sigma $
%\Ensure $\text{process grid: } g_M, g_N, g_K: g_M g_N g_K \le p \text{, domain 
%size adjusted to problem dimensions: } a_M, a_N, b $
%%
%\State $g_M, g_N, g_K \gets \infty$
%\State $a_M \gets a_{opt}, a_N \gets a_{opt}, b \gets b_{opt}$
%
%%\Comment{If the evaluation of $a_{opt}$ and $b_{opt}$ required no trunking, }
%\Statex{$\triangleright$ Iteratively find the communication-minimizing grid 
%size smaller than $p$}
%\While{$ g_M g_N g_K > p$} 
%\State  $(g_M, g_N, g_K) \gets (\left \lceil{M / a_M} \right \rceil,\left 
%\lceil{N / a_N} \right \rceil , \left \lceil{K / b} \right \rceil)$ 
%\If{$g_M \ge g_N \ge g_K$}
%\State $a_M \gets \left \lceil{M / (g_M-1)} \right \rceil$
%\ElsIf {$g_N \ge g_M \ge g_K$}
%\State $a_N \gets \left \lceil{N / (g_N-1)} \right \rceil$
%\Else
%\State $b \gets \left \lceil{K / (g_K-1)} \right \rceil$
%\EndIf
%\EndWhile
%\State $CommVol_1 \gets a_N \cdot a_M + a_N \cdot b + a_M \cdot b$ 
%\Comment{Communication volume per process}
%\State $CompVol_1 \gets a_N \cdot a_M \cdot b$ \Comment{Computation volume per 
%process}
%
%\Statex{$\triangleright$ Find best resource-fitting process grid}
%\State $f \gets Factorize(p)$
%\State $(g_{M,2}, g_{N,2}, g_{K,2}) \gets FitFactors(f,M,N,K)$ 
%\State $(a_{M,2}, a_{N,2}, b_2) \gets (\left \lceil{M / g_{M,2}} \right 
%\rceil,\left \lceil{N / g_{N,2}} \right \rceil,\left \lceil{K / g_{K,2}} 
%\right 
%\rceil)$
%\State $CommVol_2 = a_{M,2} \cdot a_{N,2} + a_{M,2}\cdot b_2 + a_{N,2} \cdot 
%b_2$ \Comment{Communication volume per process}
%\State $CompVol_2 = a_{M,2} \cdot a_{N,2} \cdot b_2$ \Comment{Computation 
%volume per process}
%
%\If{$\sigma \cdot CommVol_1 + CompVol_1 > \sigma \cdot CommVol_2 + CompVol_2$}
%\Comment{Resource fitting grid has lower cost. Use this grid instead}
%\State $(a_{M}, a_{N}, b) \gets (a_{M,2}, a_{N,2}, b_2)$
%\State $(g_{M}, g_{N}, g_K) \gets (g_{M,2}, g_{N,2}, g_{K,2})$
%\EndIf
%\end{algorithmic}
%\end{algorithm}
%\end{figure}
