...
%	
%	DEFINITION 2:  (S-partitioning of a CDAG \cite{redbluewhite}).
...
%
%\textit{Claim:}
%In decision problem $\mathcal{I}$, each of the gadgets is contained in a 
%single 
%S-partition subset. 
...
%opposite that there is a valid S-partition in which there is a gadget 
%contained 
...
%output 
%vertices $w_{i}$, $0 < a + b + c < 2B + 1 + s(i)$ belong to subset $p1$ and 
%the 
...
%p(g(i))$, where $b(i)$ is a bin number in which item $i$ is placed, $g(i)$ is 
%a 
