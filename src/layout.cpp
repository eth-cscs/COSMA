...
// if BFS and DFS are interleaved
...
int Layout::offset(int prev_pointer) {
    return offset(rank_, prev_pointer);
}

void Layout::next(int rank) {
    pointer_[rank]++;
}
...
std::vector<int> Layout::dfs_buckets(Interval& newP) {
...
void Layout::set_dfs_buckets(Interval& newP, std::vector<int>& pointers) {
    for (int i = newP.first(); i <= newP.last(); ++i) {
        pointer_[i] = pointers[i - newP.first()];
    }
}

int Layout::dfs_bucket(int rank) {
    return pointer_[rank];
}

int Layout::dfs_bucket() {
    return dfs_bucket(rank_);
}
...
void Layout::buffers_before_expansion(Interval& P, Interval2D& range, 
...
void Layout::buffers_after_expansion(Interval& P, Interval& newP,
        std::vector<std::vector<int>>& size_per_rank,
        std::vector<int>& total_size_per_rank,
...
void Layout::set_sizes(Interval& newP, std::vector<std::vector<int>>& size_per_rank, int offset) {
    for (int i = newP.first(); i <= newP.last(); ++i) {
        set_sizes(i, size_per_rank[i - newP.first() + offset], pointer_[i]);
    }
}

void Layout::set_sizes(Interval& newP, std::vector<std::vector<int>>& size_per_rank) {
    for (int i = newP.first(); i <= newP.last(); ++i) {
...
void Layout::set_sizes(int rank, std::vector<int>& sizes, int start) {
    ...
    for (int i = pointer; i < std::min(sizes.size() + pointer, b_sizes.size()); ++i) {
...
std::vector<int> Layout::sizes_inside_range(Interval2D& range, int rank, int& total_size) {
