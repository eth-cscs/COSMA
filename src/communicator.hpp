...
public:
     ...
     * in sequential steps (only in sequential steps) in which this matrix was split. 
    ...
    virtual void overlap_comm_and_comp(context& ctx, CosmaMatrix& matrixA, CosmaMatrix& matrixB,
        CosmaMatrix& matrixC, Interval& m, Interval& n, Interval& k, Interval& P,
        size_t step, double beta) = 0;
...
protected:
    ...
    std::tuple<MPI_Group, MPI_Comm> create_comm_ring(MPI_Comm comm, Interval& P, int offset, int div);
    std::tuple<MPI_Group, MPI_Comm> create_comm_subproblem(MPI_Comm comm, Interval& P, Interval& newP);
