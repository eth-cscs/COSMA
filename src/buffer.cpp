...
void Buffer::initialize_buffers(bool dry_run) {
    ...
    if (!dry_run) {
        ...
        // std::cout << "buff sizes for " << label_ << " = " << buff_sizes.size() << std::endl;
        ...
        // std::cout << std::endl;
        // std::cout << "Initial size = " << mapper_->initial_size() << std::endl;
        buffers_[0].resize(std::max((size_t) buff_sizes[0], mapper_->initial_size()));

        // std::cout << "matrix = " << label_ << ", # buffers = " << buffers_.size() << std::endl;
...
void Buffer::init_first_split_steps() {
    ...
    while (step < strategy_->n_steps) {
        if (strategy_->sequential_step(step) && strategy_->split(label_, step)) {
            // split in seq
        ...
        } else if (strategy_->parallel_step(step) && !strategy_->split(label_, step)) {
            // expanded
...
int Buffer::buff_index_before_gemm() const { 
    ...
    // std::cout << "par steps before gemm for " << label_ << " = " << strategy_->par_steps_before_gemm(label_) << std::endl;
    return strategy_->parallel_steps_before_gemm(label_) % 2 != 0 ? buffers_.size() - 1 : buffers_.size() - 2;
}
...
std::vector<long long> Buffer::compute_buffer_size(Interval& m, Interval& n, Interval& k, 
    ...
    // of the current submatrices. We say "before" because whenever we split in sequential
    ...
    if (strategy_->sequential_step(step)) {
        ...
        for (int i = 0; i < div; ++i) {
            ...
            // finds the maximum buffer size for each step among all sequential branches
        ...
        if (strategy_->split(label_, step)) {
            int max_size = 0;
            std::vector<int> block_sizes = layout_->sizes_inside_range(range, rank_, max_size);
        ...
        if (expanded) {
            ...
            */
            ...
            // if (label_ == 'C') {
                ...
                std::tie(subint_index, subint_offset) = P.locate_in_subinterval(div, rank);
                int target = P.locate_in_interval(div, subint_index, subint_offset);
