...
public:
    ...
    // otherwise, uses polling to query if the communication request has completed
    ...
    // if there is enough memory uses a parallel step, if not uses a sequential step
    ...
    // if there is not enough memory in some step, then a sequential step is performed and new
...
private:
    bool divide(std::vector<int>& div_factors, int& dim_i,
