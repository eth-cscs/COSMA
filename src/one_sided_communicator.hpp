...
public:
    static MPI_Win create_window(MPI_Comm comm, double* pointer, size_t size, bool no_locks) {
        MPI_Info info ;
    ...
    static void copy(MPI_Comm comm, int rank, int div,
            Interval& P, double* in, double* out,
    ...
    static void comm_task_mn_split_polling(int divisor, int gp,
            double* original_matrix, double* expanded_matrix,
...
#pragma omp single nowait
        for (int dist = 1; dist < divisor; dist++) {
            ...
            MPI_Get(expanded_matrix + k.length() * displacements_n[rank], b_size, MPI_DOUBLE, rank,
                    0, b_size, MPI_DOUBLE, win);
...
#pragma omp critical
                {
                    ...
                    double* pointer_b = expanded_matrix + k.length() * displacements_n[rank];
                    double* pointer_c = prev_c + newm.length() * displacements_n[rank];

                    matrixB.set_current_matrix(pointer_b);
                    matrixC.set_current_matrix(pointer_c);

                    PL();
                    local_multiply_cpu(matrixA.current_matrix(), matrixB.current_matrix(),
                            matrixC.current_matrix(), newm.length(), 
                            n.subinterval(divisor, rank).length(), k.length(), beta);
                    // local_multiply(ctx, matrixA.current_matrix(), matrixB.current_matrix(), 
                    //         matrixC.current_matrix(), newm.length(), 
                    //         n.subinterval(divisor, rank).length(), k.length(), beta);
                    PE(multiply_communication_copy);
...
*/
    ...
    static void overlap_m_split(context& ctx, MPI_Comm comm, int rank, int divisor,
        ...
        std::thread comm_thread(
                ...
                std::ref(displacements_n), std::ref(ready), comm);
        ...
        while (dist < divisor) {
            while (ready > 0) {
                ...
                // Compute the piece that has arrived
                double* pointer_b = expanded_matrix + k.length() * displacements_n[idx];
                double* pointer_c = prev_c + newm.length() * displacements_n[idx];
    ...
    static void overlap_n_split(context& ctx, MPI_Comm comm, int rank, int divisor,
        ...
        std::vector<double> block_buffer(newn.length() * math_utils::int_div_up(k.length(), divisor));
        // std::cout << "block buffer size = " << block_buffer.size() << std::endl;

        std::atomic_int ready(1);
        std::thread comm_thread(
                communicator::use_busy_waiting ? comm_task_mn_split_busy_waiting : comm_task_mn_split_polling,
                divisor, gp, original_matrix, expanded_matrix, m, k,
                std::ref(displacements_k), std::ref(ready), comm);
        ...
        while (dist < divisor) {
            while (ready > 0) {
                ...
                // Compute the piece that has arrived
                double* pointer_a = dist == 0 ? original_matrix : (expanded_matrix + m.length() * displacements_k[idx]);
                // double* pointer_b = switch_buffers ? buffer2.data() : buffer1.data();
    ...
    static void comm_task_k_split(int divisor, int gp, int off, int jump_size,
            double* expanded_matrix, double* recv_buffer,
    ...
    static void compute(context& ctx, CosmaMatrix& A, CosmaMatrix& B, CosmaMatrix& C, 
            double* pointer_b, double* pointer_c,
            Interval& m, Interval& n, Interval& k, std::vector<int>& displacements_n, double beta, int start, int end) {
        if (start >= end) return;
    ...
    static void overlap_k_split(context& ctx, MPI_Comm comm, int rank, int divisor,
        ...
        std::thread comm_task(comm_task_k_split, divisor, gp, off, target_jump_size,
                expanded_matrix, original_matrix, m, n, P,
                std::ref(displacements_n), std::ref(ready),
                std::ref(mtx), std::ref(cv), comm);
        ...
        if (target_jump_size == divisor) {
            compute(ctx, matrixA, matrixB, matrixC, prev_b, prev_c, m, 
                n, newk, std::ref(displacements_n), beta, 0, divisor);
            ...
            while (processed < divisor) {
                ...
                if (start < end) {
                    ...
                    compute(ctx, matrixA, matrixB, matrixC, prev_b, prev_c, m, 
                        n, newk, std::ref(displacements_n), beta, start, end);
                    ...
                    if (processed < divisor) {
                        ...
                        if (next_end <= divisor) {
                            compute(ctx, matrixA, matrixB, matrixC, prev_b, prev_c, m, 
                                n, newk, std::ref(displacements_n), beta, end, next_end);
                            processed++;
                    ...
                    compute(ctx, matrixA, matrixB, matrixC, prev_b, prev_c, m, 
                        n, newk, std::ref(displacements_n), beta, start, divisor);
                    compute(ctx, matrixA, matrixB, matrixC, prev_b, prev_c, m, 
                        n, newk, std::ref(displacements_n), beta, 0, end);
    ...
    static void overlap_comm_and_comp(context& ctx, MPI_Comm comm, int rank, const Strategy * strategy,
        CosmaMatrix& matrixA, CosmaMatrix& matrixB, CosmaMatrix& matrixC,
        Interval& m, Interval& n, Interval& k, Interval& P, size_t step, double beta) {

        int divisor = strategy->divisor(step);
        if (strategy->split_m(step)) {
            one_sided_communicator::overlap_m_split(ctx, comm, rank, divisor,
                    matrixA, matrixB, matrixC, m, n, k, P, beta);
        } else if (strategy->split_n(step)) {
            one_sided_communicator::overlap_n_split(ctx, comm, rank, divisor,
                    matrixA, matrixB, matrixC, m, n, k, P, beta);
        } else {
            one_sided_communicator::overlap_k_split(ctx, comm, rank, divisor,
                    matrixA, matrixB, matrixC, m, n, k, P, beta);
        }
    }
};
