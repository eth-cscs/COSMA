...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i < divisor; ++i) {
        if (i != gp) {
            ...
            MPI_Recv_init(global_buffer.data() + offset, local_size, MPI_DOUBLE,
                    i, 0, subcom, &req[reqi]);
            MPI_Send_init(local_buffer.data(), local_size, MPI_DOUBLE, i, 0, 
                    subcom, &req[divisor - 1 + reqi]);
            reqi++;
        }
    }
...

    {
        ...
        MPI_Allgather(local_buffer.data(), local_size, MPI_DOUBLE, 
                global_buffer.data(), local_size, MPI_DOUBLE, subcom);
